...
def graph_method(matrix, pos_threshold=0.5) -> Tuple[List[List[int]], List[List[int]]]:
    ...
    for i in range(len(indices)):
        for j in range(i + 1, len(indices)):
            if matrix[indices[i], indices[j]] > pos_threshold:
                pos_graph.add_edge(indices[i], indices[j])
            if matrix[indices[i], indices[j]] < neg_threhold:
                neg_graph.add_edge(indices[i], indices[j])
...
def place_segment(resonate):
    """Place resonant segment to the best position, this is related to your storage system"""
    pass